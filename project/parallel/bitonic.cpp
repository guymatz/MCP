...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  for (subArraySize = 2; subArraySize <= n; subArraySize = subArraySize * 2) {
    // cout << "SUBaRRAYsIZE" << "\t" << "distance" << "\t" << "startIdx" <<
    // "\t" << "swapPartner" << std::endl;
    //  we break the problem into "sub array" halves
    for (distance = subArraySize / 2; distance > 0; distance = distance / 2) {
      // cout << "Thread ID" << "\t" << "subArraySize" << "\t" << "DISTANCE" <<
      // "\t" << "startIdx" << "\t" << "swapPartner" << std::endl;
...
#pragma omp parallel for private(swapPartner)
      for (startIdx = 0; startIdx < n; startIdx++) {
        ...
        if (swapPartner > startIdx) {
          // cout << omp_get_thread_num() << "\t\t" << subArraySize << "\t\t" <<
          // distance << "\t\t" << startIdx << "\t\t" << swapPartner <<
          // std::endl << std::endl;
          ...
          if ((((int)floor(startIdx / subArraySize) % 2) != 0) &&
              Vnums[startIdx] < Vnums[swapPartner]) {
            // cout << "!=0 : " << startIdx << " " << subArraySize << " " <<
            // (startIdx&subArraySize) << std::endl;
